...
function mostrarContenido(idContenido) {
    ...
    if (contenidoSeleccionado) {
        ...
        // Clonar el contenedor de recursos si no existe en la lección actual
        const recursosContainer = contenidoSeleccionado.querySelector('.resources-container');
        if (!recursosContainer) {
            const recursosTemplate = document.querySelector('#videoIntroductorio .resources-container');
            if (recursosTemplate) {
                const recursosClone = recursosTemplate.cloneNode(true);
                contenidoSeleccionado.appendChild(recursosClone);
            }
        }

        // Clonar el contenedor de quiz si no existe en la lección actual
        const quizContainer = contenidoSeleccionado.querySelector('.quiz-container');
        if (!quizContainer) {
            const quizTemplate = document.querySelector('#videoIntroductorio .quiz-container');
            if (quizTemplate) {
                const quizClone = quizTemplate.cloneNode(true);
                contenidoSeleccionado.appendChild(quizClone);
            }
        }
    }
}
